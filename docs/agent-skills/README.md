# Agent skills — ollamatoolkit

Install structured agent workflows for **ollamatoolkit** across mainstream coding agents (Cursor, Claude Code, Gemini CLI, Antigravity, GitHub Copilot, Windsurf, OpenCode, Kiro).
...
```powershell
./scripts/install-agent-skills.ps1 -RepoUrl "https://github.com/imyourboyroy/ollamatoolkit" -Agent all
```

**macOS / Linux:**

```bash
./scripts/install-agent-skills.sh --repo-url "https://github.com/imyourboyroy/ollamatoolkit" --agent all
```
...
| Skill | Purpose |
|-------|---------|
| `ollamatoolkit` | OllamaClient vs SimpleAgent vs MCPToolManager — when to use each layer |

Read `AGENTS.md` for full repo rules. Requires a reachable Ollama server (`OLLAMA_HOST` / `base_url`).
...
```text
skills/ollamatoolkit/SKILL.md   # skill entry point
